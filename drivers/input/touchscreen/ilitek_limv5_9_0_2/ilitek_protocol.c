...
int api_protocol_get_mcuver(uint8_t *inbuf, uint8_t *outbuf)
{
	...
	memcpy(ilitek_data->mcu_ver, outbuf, 5);
...
int api_protocol_get_key_info_v3(uint8_t *inbuf, uint8_t *outbuf)
{
	int ret = ILITEK_SUCCESS, i = 0;
	uint8_t buf[64] = {0};
...
int api_protocol_get_tp_resolution_v3(uint8_t *inbuf, uint8_t *outbuf)
{
	int ret = ILITEK_SUCCESS;
	uint8_t buf[64] = {0};
	...
	if (ilitek_data->keycount > ILITEK_SUPPORT_MAX_KEY_CNT) {
		tp_log_err("exception keycount %d > %d\n", ilitek_data->keycount, ILITEK_SUPPORT_MAX_KEY_CNT);
		return -EINVAL;
	} else if (ilitek_data->keycount > 0) {
		ret = api_protocol_get_key_info_v3(NULL, outbuf);
		if (ret < 0)
			return ret;
	}
...
//--------------------------protocol v6-------------------------//
int api_protocol_get_key_info_v6(uint8_t *inbuf, uint8_t *outbuf)
{
	int ret = ILITEK_SUCCESS, i = 0;
	uint8_t buf[64] = {0};
	int key_num = ilitek_data->keycount;
...
int api_protocol_get_tp_resolution_v6(uint8_t *inbuf, uint8_t *outbuf)
{
	int ret = ILITEK_SUCCESS;
	uint8_t buf[64] = {0};
	...
	if (ilitek_data->keycount > ILITEK_SUPPORT_MAX_KEY_CNT) {
		...
		ret = api_protocol_get_key_info_v6(NULL, outbuf);
		if (ret < 0)
			return ret;
	}
	if (ilitek_data->max_tp > 40) {
		tp_log_err("exception max_tp > 40 is %d set max_tp = 40\n", ilitek_data->max_tp);
...
{
	if (ilitek_data->ptl.flag == PTL_V3)
		return api_protocol_get_key_info_v3(inbuf, outbuf);
	else if (ilitek_data->ptl.flag == PTL_V6)
		return api_protocol_get_key_info_v6(inbuf, outbuf);
	else
		return -EINVAL;
}
...
{
	if (ilitek_data->ptl.flag == PTL_V3)
		return api_protocol_get_tp_resolution_v3(inbuf, outbuf);
	else if (ilitek_data->ptl.flag == PTL_V6)
		return api_protocol_get_tp_resolution_v6(inbuf, outbuf);
	else
		return -EINVAL;
}
...
int api_set_short_Info(int32_t Dump1, int32_t Dump2,uint8_t verf, uint8_t posidleL, uint8_t posidleH)
{
	...
	if (api_protocol_set_cmd(SET_SHORT_INFO, inbuf, NULL) < 0) {
		return ILITEK_FAIL;
	}
	return ILITEK_SUCCESS;
}
...
int api_set_open_Info(uint8_t freqL, uint8_t freqH,uint8_t gain)
{
	...
	if (api_protocol_set_cmd(SET_OPEN_INFO, inbuf, NULL) < 0) {
		return ILITEK_FAIL;
	}
	return ILITEK_SUCCESS;
}
...
uint16_t api_get_block_crc(uint32_t start, uint32_t end, uint32_t type) {
	...
	inbuf[6] = (end >> 8) & 0xFF;
	inbuf[7] = (end >> 16) & 0xFF;
	if (api_protocol_set_cmd(GET_BLK_CRC, inbuf, outbuf) < 0) {
		return ILITEK_FAIL;
	}
...
int api_protocol_set_cdc_initial(uint8_t *inbuf, uint8_t *outbuf)
{
	...
	inbuf[0] = ILITEK_TP_CMD_SET_CDC_INITIAL_V6;
	ret = ilitek_i2c_write_and_read(inbuf, 2, 10, outbuf, 0);
	return ret;
}

/* Private define ------------------------------------------------------------*/
PROTOCOL_MAP ptl_func_map[] =
{
	/* common cmds */
	[GET_PTL_VER] = {ILITEK_TP_CMD_GET_PROTOCOL_VERSION, api_protocol_get_ptl, "GET_PTL_VER", PTL_V3 | PTL_V6},
	[GET_FW_VER] = {ILITEK_TP_CMD_GET_FIRMWARE_VERSION, api_protocol_get_fwver, "GET_FW_VER", PTL_V3 | PTL_V6},
	[GET_SCRN_RES] = {ILITEK_TP_CMD_GET_SCREEN_RESOLUTION, api_protocol_get_sc_resolution, "GET_SCRN_RES", PTL_V3 | PTL_V6},
	[GET_TP_RES] = {ILITEK_TP_CMD_GET_TP_RESOLUTION, api_protocol_get_tp_resolution,"GET_TP_RES", PTL_V3 | PTL_V6},
	[GET_MCU_MOD] = {ILITEK_TP_CMD_READ_MODE, api_protocol_check_mode, "GET_MCU_MODE", PTL_V3 | PTL_V6},
	[GET_MCU_VER] = {ILITEK_TP_CMD_GET_KERNEL_VERSION, api_protocol_get_mcuver, "GET_MCU_VER", PTL_V3 | PTL_V6},
	[GET_KEY_INFO] = {ILITEK_TP_CMD_GET_KEY_INFORMATION, api_protocol_get_key_info, "GET_KEY_INFO", PTL_V3 | PTL_V6},
	[SET_IC_SLEEP] = {ILITEK_TP_CMD_SET_SLEEP, api_protocol_set_sleep, "SET_IC_SLEEP", PTL_V3 | PTL_V6},
	[SET_IC_WAKE] = {ILITEK_TP_CMD_SET_WAKEUP, api_protocol_set_wakeup, "SET_IC_WAKE", PTL_V3 | PTL_V6},
	[SET_FUNC_MOD] = {ILITEK_TP_CMD_SET_FUNC_MODE, api_protocol_control_funcmode, "SET_FUNC_MOD", PTL_V3 | PTL_V6},
	[GET_SYS_BUSY] = {ILITEK_TP_CMD_GET_SYSTEM_BUSY, api_protocol_system_busy, "GET_SYS_BUSY", PTL_V3 | PTL_V6},
	[SET_AP_MODE] = {ILITEK_TP_CMD_SET_APMODE, api_protocol_set_apmode, "SET_AP_MODE", PTL_V3 | PTL_V6},
	[SET_BL_MODE] = {ILITEK_TP_CMD_SET_BLMODE, api_protocol_set_blmode, "SET_BL_MODE", PTL_V3 | PTL_V6},

	/* v3 only cmds */
	[SET_WRITE_EN] = {ILITEK_TP_CMD_WRITE_ENABLE, api_protocol_write_enable, "SET_WRITE_EN", PTL_V3},
	[SET_TEST_MOD] = {ILITEK_TP_CMD_CONTROL_TESTMODE, api_protocol_control_tsmode, "SET_TEST_MODE", PTL_V3},

	/* v6 only cmds */
	[SET_MOD_CTRL] = {ILITEK_TP_CMD_SET_MODE_CONTORL, api_protocol_set_mode, "SET_CTRL_MODE", PTL_V6},
	[SET_W_FLASH] = {ILITEK_TP_CMD_WRITE_FLASH_ENABLE, api_protocol_write_flash_enable, "SET_WRITE_FLASH", PTL_V6},
	[GET_BLK_CRC] = {ILITEK_TP_CMD_GET_BLOCK_CRC, api_protocol_get_block_crc, "GET_BLK_CRC", PTL_V6},
	[SET_CDC_INIT] = {ILITEK_TP_CMD_SET_CDC_INITIAL_V6, api_protocol_set_cdc_initial, "SET_CRC_INIT", PTL_V6},
	[SET_SHORT_INFO] = {ILITEK_TP_CMD_SET_SHORT_INFO, api_protocol_set_short_Info, "SET_SHORT_INFO", PTL_V6},
	[SET_OPEN_INFO] = {ILITEK_TP_CMD_SET_OPEN_INFO, api_protocol_set_open_Info, "SET_OPEN_INFO", PTL_V6},
};

//-----------------------extern api function------------------------//
int api_protocol_set_cmd(uint16_t idx, uint8_t *inbuf, uint8_t *outbuf)
{
	int ret = ILITEK_SUCCESS;
	...
	if (!(ilitek_data->ptl.flag & ptl_cb_func[idx].flag)) {
		tp_log_err("Unexpected cmd: %s for %x only, now is %x\n",
			ptl_cb_func[idx].name, ptl_cb_func[idx].flag,
			ilitek_data->ptl.flag);
		return -EINVAL;
	}

	ret = ptl_cb_func[idx].func(inbuf, outbuf);
	if (ret < 0)
		return ret;
...
int api_protocol_set_testmode(bool testmode)
{
	...
	if(ilitek_data->ptl.flag == PTL_V3) {
		...
		if (api_protocol_set_cmd(SET_TEST_MOD, inbuf, NULL) < 0) {
			return ILITEK_FAIL;
		}
		mdelay(10);
	} else if(ilitek_data->ptl.flag == PTL_V6) {
...
int api_protocol_init_func(void){
	...
	if (api_protocol_set_cmd(GET_PTL_VER, NULL, outbuf) < ILITEK_SUCCESS) {
		tp_log_info("\n");
		return ILITEK_FAIL;
	}
