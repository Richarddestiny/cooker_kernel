...
uint32_t get_ap_endaddr(uint32_t startAddr,uint32_t endAddr, uint8_t input[]) {
	...
	addr = findstr(input, startAddr, endAddr, tag, 32);
	if(addr == 0)
		addr = endAddr;
...
static int32_t ilitek_changetoblmode(bool mode)
{
	int32_t i = 0;
	uint8_t outbuf[64];
	if(api_protocol_set_cmd(GET_MCU_MOD, NULL, outbuf) < ILITEK_SUCCESS)
		return ILITEK_I2C_TRANSFER_ERR;
	msleep(30);
	...
	if ((outbuf[0] == ILITEK_TP_MODE_APPLICATION && !mode) || (outbuf[0] == ILITEK_TP_MODE_BOOTLOADER && mode)) {
		...
		for (i = 0; i < 5; i++) {
			if((ilitek_data->bl_ver[0] == 0x1 && ilitek_data->bl_ver[1] >= 0x8) || (ilitek_data->ptl.ver_major == 0x6)) {
				if(api_protocol_set_cmd(SET_W_FLASH, NULL, outbuf) < ILITEK_SUCCESS) {
					return ILITEK_I2C_TRANSFER_ERR;
				}
			}
			else {
				if(api_protocol_set_cmd(SET_WRITE_EN, NULL, outbuf) < ILITEK_SUCCESS) {
					return ILITEK_I2C_TRANSFER_ERR;
				}
			}
			...
			msleep(20);
			if (mode) {
				if(api_protocol_set_cmd(SET_BL_MODE, NULL, outbuf) < ILITEK_SUCCESS) {
					return ILITEK_I2C_TRANSFER_ERR;
				}
			} else {
				if(api_protocol_set_cmd(SET_AP_MODE, NULL, outbuf) < ILITEK_SUCCESS) {
					return ILITEK_I2C_TRANSFER_ERR;
				}
			}
			msleep(500 + i*100);
...
Retry:
	...
	//check ic type
	if (api_protocol_set_cmd(GET_MCU_VER, NULL, buf) < ILITEK_SUCCESS)
		goto Retry;
	...
	if(update) {
		...
		if (api_protocol_set_cmd(GET_PTL_VER, NULL, buf) < ILITEK_SUCCESS)
			goto Retry;
...
int32_t ilitek_boot_upgrade_3XX(void)
{
	...
	if ((ilitek_data->upgrade_mcu_ver[0] != 0 || ilitek_data->upgrade_mcu_ver[1] != 0)
		...
		ret = api_protocol_set_cmd(GET_MCU_VER, NULL, buf);
...
int32_t ilitek_boot_upgrade_6XX(void)
{
	...
	if (!update_buf) {
		tp_log_err("update_buf allocate failed\n");
		goto error_return;
	}
	...
	if(ilitek_data->upg.hex_ic_type[0] != ilitek_data->mcu_ver[0] &&
			ilitek_data->upg.hex_ic_type[1] != ilitek_data->mcu_ver[1]) {
		tp_log_err("ic and hex no match, ic:%x  hex:%x\n",
...
int32_t ilitek_bin_upgrade_3XX(struct device *dev, const char *fn) {
	...
	if (!CTPM_FW) {
		tp_log_err("CTPM_FW vmalloc failed\n");
		ret = -ENOMEM;
		goto error_release;
	}
