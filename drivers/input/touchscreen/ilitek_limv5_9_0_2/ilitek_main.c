...
static void ilitek_esd_check(struct work_struct *work)
{
	...
	if (ilitek_data->esd_check) {
		for (i = 0; i < 3; i++) {
			if (ilitek_i2c_write_and_read(buf, 1, 5, buf, 2) < 0) {
				...
				if (buf[0] == ilitek_data->ptl.ver_major) {
...
void ilitek_suspend(void)
{
	...
	if (api_protocol_set_cmd(SET_IC_SLEEP, NULL, NULL) < ILITEK_SUCCESS)
...
#if ILITEK_LOW_POWER == ILITEK_SLEEP
	if (api_protocol_set_cmd(SET_IC_WAKE, NULL, NULL) < ILITEK_SUCCESS)
		tp_log_err("0x31 set wake up err\n");
#endif
	//If ILITEK_SLEEP is defined and FW support wakeup command, the reset can be mark.
	ilitek_reset(ilitek_data->reset_time, false);
	mutex_unlock(&ilitek_data->ilitek_mutex);
	ilitek_irq_enable();
	ilitek_data->system_suspend = false;
...
int ilitek_read_tp_info(bool boot)
{
	...
	if (api_protocol_set_cmd(GET_PTL_VER, NULL, outbuf) < ILITEK_SUCCESS)
		goto transfer_err;
	if (api_protocol_set_cmd(GET_MCU_VER, NULL, outbuf) < ILITEK_SUCCESS)
		goto transfer_err;
#ifdef ILITEK_GESTURE
	ilitek_data->enable_gesture = true;
#endif
	if (api_protocol_set_cmd(GET_FW_VER, NULL, outbuf) < ILITEK_SUCCESS)
		goto transfer_err;
	if (boot && api_protocol_set_cmd(GET_SCRN_RES ,NULL, outbuf) < ILITEK_SUCCESS)
			goto transfer_err;
	if (api_protocol_set_cmd(GET_TP_RES, NULL, outbuf) < ILITEK_SUCCESS)
		goto transfer_err;
...
#ifdef ILITEK_ENABLE_DMA
		if (I2CDMABuf_va) {
			dma_free_coherent(&tpd->dev->dev, ILITEK_DMA_SIZE, I2CDMABuf_va, I2CDMABuf_pa);
...
#endif
		if (ilitek_data->blk)
			kfree(ilitek_data->blk);
		if (ilitek_data->ic)
			kfree(ilitek_data->ic);
		kfree(ilitek_data);
		ilitek_data = NULL;
	}
	return 0;
