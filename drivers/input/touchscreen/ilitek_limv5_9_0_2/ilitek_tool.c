...
#endif
{
	static uint8_t *buffer = NULL; //[64] = { 0 };
...
int hex_mapping_convert(uint32_t addr,uint8_t *buffer) {
	...
	if(ilitek_data->upg.map_ver >= 0x10000)
	{
		...
		for(count = 0; count < ilitek_data->upg.blk_num; count++)
		{
			...
			else if(ilitek_data->blk[count].start > ILITEK_HEX_UPGRADE_SIZE - 32)
				;
			else
				ilitek_data->blk[count].end = get_block_endaddr(ilitek_data->blk[count].start, ilitek_data->blk[count].end, buffer);

			tp_log_info("Block:%d, start:0x%x end:0x%x(Tag)\n", count, ilitek_data->blk[count].start,  ilitek_data->blk[count].end);
		}
	}
	return ILITEK_SUCCESS;
}
...
static ssize_t ilitek_update_with_hex_read(struct file *pFile, char __user *buf, size_t nCount, loff_t *pPos)
{
	...
	if (api_protocol_set_cmd(GET_MCU_VER, NULL, outbuf) < ILITEK_SUCCESS) {
		...
		api_protocol_set_cmd(GET_FW_VER, NULL, outbuf);
		...
		if (ilitek_data->upgrade_FW_info_addr != -1 &&
				(ilitek_data->mcu_ver[0] != CTPM_FW[ilitek_data->upgrade_FW_info_addr + 32 + 6] ||
...
static ssize_t ilitek_module_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ilitek_irq_disable();
	mutex_lock(&ilitek_data->ilitek_mutex);
	ret = api_protocol_set_cmd(GET_MCU_VER, NULL, tmpbuf);
	mutex_unlock(&ilitek_data->ilitek_mutex);
	ilitek_irq_enable();
...
static ssize_t ilitek_update_fw_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0, i = 0;
...
static DEVICE_ATTR(update_fw, 0664, NULL, ilitek_update_fw_store);
